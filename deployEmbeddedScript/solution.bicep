...
module hostPool 'modules/hostpool.bicep' = if ((!CrossTenantRegister) && (WorkspaceName != 'none')) {
...
module virtualMachines 'modules/virtualmachines.bicep' = [for i in range(1, SessionHostBatchCount): {
  ...
  params: {
    ...
    OUPath: OUPath
    Location: Location
    LogAnalyticsWorkspaceId: Monitoring ? logAnalyticsWorkspace.outputs.logAnalyticsId : 'none'
